...
def amazon_detail_html(item)
	...
	label = amazon_label(item)
	desc = ''
	if author == "-"
		desc = "#{h label}"
	else
		desc = "#{h author}, #{h label}"
	end
	price = amazon_price(item)
	unless price == "(no price)"
		desc = desc + ", #{h price}"
	end
	...
	<span class="amazon-detail">
		<amp-img src="#{h image[:src]}" height="#{h image[:height]}" width="#{h image[:width]}" alt="#{h title}" layout="fixed"></amp-img>
		<span class="amazon-detail-desc">
		<span class="amazon-title">#{h title}</span><br/>
		<span class="amazon-label">#{h desc}</span>
	</span>
	</span>
	</a>
	HTML
...
def amazon_to_html(item, with_image = true, label = nil, pos = 'amazon')
	...
	if with_image
		...
		unless image[:src] then
			...
			img = <<-HTML
			<amp-img class="#{h pos}" src="#{h image[:src]}"
			#{size} alt="#{h alt}" layout="responsive"></amp-img>
			HTML
			img.gsub!( /\t/, '' )
		end
	end
...
def amazon_get(asin, with_image = true, label = nil, pos = 'amazon')
	...
	begin
		...
		@logger.error "amp_amazon.rb: Amazon API Unauthorized."
		message = asin
		...
		@logger.error "amp_amazon.rb: PA-API Timeouted."
		message = asin
		if @mode == 'preview' then
		...
		@logger.error "amp_amazon.rb: #{e.message}"
		message = ''
		# Handle 429 "Too Many Requests"
		if /^429/ =~ e.message then
			url = "https://www.amazon.co.jp/dp/#{h asin}"
			label ||= url
			message << %Q|<a href="#{h url}?tag=#{@conf['amazon.aid']}&linkCode=ogi&th=1&psc=1">#{h label}</a>|
		end
		if @mode == 'preview' then
		...
		@logger.error "amp_amazon.rb: #{json["Errors"][0]["Message"]}"
		message = label || asin
		if @mode == 'preview' then
