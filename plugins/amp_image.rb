...
def image(src)
	w, h = FastImage.size("#{src}.png")
	r =  %Q| <article> |
	r += %Q|   <amp-img layout="responsive" width="#{w}" height="#{h}" src="#{src}.webp">|
   r += %Q|     <amp-img fallback layout="responsive" width="#{w}" height="#{h}" src="{#src}".png"></amp-img>|
