# coding: utf-8
# amp_image.rb from image_plugin_ex.rb
# version 0.1
# -pv-
#
# 名称:
...
# Copyright (c) 2026 Youhei SASAKI <uwabami@gfd-dennou.org>
#
# You can redistribute it and/or modify it under GPL2.
#
=begin Changelog
2026-03-01 Youhei SASAKI <uwabami@gfd-dennou.org>
	* update for amp html
...
def image( id, alt = "image", id2 = nil, width = nil, place="none" )
	...
	# 	width_tag = ""
	# end
	f = "#{image_dir}/#{list[id.to_i]}"
	r = ""
	begin
		unless File.directory?(f)
			info = FastImage.new( f )
			w, h = info.size
			width_tag = %Q|width="#{w}" height="#{h}" style="max-width: 60vw; margin: 0 auto;"|
		else
			width_tag = ""
		end
		# r += %Q|   <amp-img layout="responsive" width="#{w}" height="#{h}" src="#{image_url}">|
		# r += %Q|     <amp-img fallback layout="responsive" width="#{w}" height="#{h}" src="{#src}"></amp-img>|
		# r += %Q|     <noscript> <img src="image_ex/20230805_01.png" width="#{w}" height="#{h}"> </noscript> |
		# r += %Q|   </amp-img> |
		if id2
			r += %Q|<amp-img layout="responsive" #{width_tag} src="#{h image_url}#{h list[id2.to_i]}" alt="#{h alt}"> |
			r += %Q|  <noscript><img src="#{h image_url}#{h list[id2.to_i]}" #{width_tag} alt="#{h alt}"></noscript>  |
			r += %Q|</amp-img> |
		else
			# if slist[id.to_i]
			# 	r += %Q|<amp-img layout="responsive" #{width_tag} src="#{h image_url}#{h list[id.to_i]}" alt="#{h alt}" class="image-ex #{h place}"> |
			# 	r += %Q|  <noscript><img src="#{h image_url}#{h list[id2.to_i]}" #{width_tag} alt="#{h alt}"></noscript>  |
			# 	r += %Q|</amp-img> |
			# else			
			if list[id.to_i]
...
#		ENV['PATH'] = nil
	...
	if @conf.respond_to?(:style) and @conf.style =~ /(Wiki|GFM)$/i
		image_plugin_tag1 = "{{"
		image_plugin_tag2 = "}}"
	elsif @conf.respond_to?(:style) and @conf.style =~ /(RD|Org)$/i
		image_plugin_tag1 = "((%"
		image_plugin_tag2 = "%))"
	else
		image_plugin_tag1 = "&lt;%="
